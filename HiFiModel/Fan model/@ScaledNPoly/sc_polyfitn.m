function polymodel = sc_polyfitn(indepvar, depvar,modelterms)
...
% Add min and max input values
for(k=1:p)
  polymodel.MinValidIn(k) = min(indepvar(:,k));
  polymodel.MaxValidIn(k) = max(indepvar(:,k));
end
...
% RMSE
polymodel.RSE_MEAN = sqrt(mean((depvar - yhat).^2));
polymodel.RSE_MAX = sqrt(max((depvar - yhat).^2));
polymodel.RSE_MEAN_PCT = sqrt(mean(((depvar - yhat)./depvar).^2))*100;
polymodel.RSE_MAX_PCT = sqrt(max(((depvar - yhat)./depvar).^2))*100;
